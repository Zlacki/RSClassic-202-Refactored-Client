...
public class DataFileDecrypter {
	...
	public static byte[] loadCachedData(String fontName, byte[] fontData) {
		int cnt = fontData[0] * 256 + fontData[1];
		int name = 0;
		fontName = fontName.toUpperCase();
		for (int cur = 0; cur < fontName.length(); cur++)
			name = name * 61 + fontName.charAt(cur) - 32;
		int l = 2 + cnt * 10;
		for (int cur = 0; cur < cnt; cur++) {
			int name_ = (fontData[(cur * 10 + 2)] & 0xFF) * 16777216
					+ (fontData[(cur * 10 + 3)] & 0xFF) * 65536
					+ (fontData[(cur * 10 + 4)] & 0xFF) * 256
					+ (fontData[(cur * 10 + 5)] & 0xFF);
			int decmp_len = (fontData[(cur * 10 + 6)] & 0xFF) * 65536
					+ (fontData[(cur * 10 + 7)] & 0xFF) * 256
					+ (fontData[(cur * 10 + 8)] & 0xFF);
			int ptr = (fontData[(cur * 10 + 9)] & 0xFF) * 65536
					+ (fontData[(cur * 10 + 10)] & 0xFF) * 256
					+ (fontData[(cur * 10 + 11)] & 0xFF);
			if (name_ == name) {
				byte[] byte_arr2 = new byte[decmp_len + 0];
				if (decmp_len != ptr)
					unpackData(byte_arr2, decmp_len, fontData, ptr, l);
				else
					for (int i2 = 0; i2 < decmp_len; i2++)
						byte_arr2[i2] = fontData[(l + i2)];
				return byte_arr2;
			}
			l += ptr;
		}
	...
	private static void method131(DataFileVariables dataFileVariables) {
		...
		while (flag19) {
			...
			flag19 = dataFileVariables.anInt486 == dataFileVariables.anInt503 + 1
					&& dataFileVariables.anInt476 == 0;
		}
	}
